...
class MySQL {
    ...
    private $callback_function; // 콜백 함수
    ...
    /**
     * @brief DB접속 또는 쿼리 실행에 오류가 발생했을때 호출될 콜백 함수를 설정한다.
     * @details 함수와 메소드를 콜백으로 설정할 수 있다.\n
     * 오류 발생시 콜백 함수에게 전달되는 인수는 MySQL 오류번호와 오류문자열이다.
     * @code
     * // 일반 함수 콜백
     * $mysql->set_error_callback('bar');
     * // 메소드 콜백
     * $foo = new foo();
     * $mysql->set_error_callback(array($foo, 'bar'));
     * // 콜백 함수측
     * function bar($errno) { echo $errno; }
     * @endcode
     * @param $function mixed 함수
     * @return mixed 이전 콜백 함수
     */
    public function set_error_callback($function) {
        $old_function = $this->callback_function;
    ...
    /**
     * @brief 콜백 함수 실행
     * @param $errno int 오류 번호
     */
    private function run_error_callback($errno, $error) {
        if ( $this->callback_function ) {
            call_user_func_array($this->callback_function, array($errno, $error));
        }
    }
    ...
    public function query($sql) {
        ...
        if ( $this->mysqli->errno ) {
            ...
            $this->rollback();
            $this->close();
            $this->run_error_callback($errno, $error);
